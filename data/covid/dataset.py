...
def get_covid_dataset(seed=0, test_prop=0.2, train_transform=None, eval_transform=None, transform=None):
    """Return stratified-by-event (train_subset, test_subset) of the COVID-CT survival dataset.
    ...
    csv_path = os.path.join('data/covid', 'clinical_processed.csv')
    img_path_full = os.path.join('data/covid', 'images')
    if not os.path.exists(csv_path):
        raise FileNotFoundError(f"Could not find {csv_path}. Please ensure data is present.")

    ds_train = CovidSurvivalDataset(csv_path, img_path_full, transform=train_transform)
    ds_eval = CovidSurvivalDataset(csv_path, img_path_full, transform=eval_transform)
...
class CovidSurvivalDataset(Dataset):
    def __init__(self, metadata_csv, img_dir, transform=None):
        self.img_dir = img_dir
        self.transform = transform

        df = pd.read_csv(metadata_csv)
        df = df[df['observed_time'] > 0]                                         # drop sentinel negatives

        self.samples = []
        print(f"Loading COVID dataset from {metadata_csv} and {img_dir}...")
        for _, row in df.iterrows():
            patient_id = row['to_patient_id']
